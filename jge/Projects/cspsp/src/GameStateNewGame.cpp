...
void GameStateNewGame::Create()
{
	...
	do {
		...
		if (strcmp(findFileData.cFileName, ".") != 0 && strcmp(findFileData.cFileName, "..") != 0) {
			name = strdup(findFileData.cFileName);
	...
	if (!gTextInput->mIsActive && !gDanzeff->mIsActive) {
		...
		if (mEngine->GetButtonClick(PSP_CTRL_SQUARE)) {
			if (!isUsingKeyboard()) {
				...
				gTextInput->Enable();
				gTextInput->mString = mSearchString;
			}
		}
		...
		if (gTextInput->mString.length() > 50) {
			gTextInput->mString = gTextInput->mString.substr(0, 50);
		}
		bool search = false;
		if (stricmp(mSearchString, (char*)gTextInput->mString.c_str()) != 0) {
			search = true;
			strcpy(mSearchString, (char*)gTextInput->mString.c_str());
		}
		if (mEngine->GetButtonClick(PSP_CTRL_START)) {
			search = true;
			gTextInput->Disable();
...
void GameStateNewGame::Render()
{
	...
	gFont->SetColor(ARGB(255,255,255,255));
	if (gDanzeff->mIsActive || gTextInput->mIsActive) {
